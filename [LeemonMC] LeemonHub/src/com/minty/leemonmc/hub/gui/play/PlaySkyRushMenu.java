...
public class PlaySkyRushMenu implements GuiBuilder
{

	private GuiUtils utils;
	private Map<Integer, ServerGroup> serversGroups = new HashMap<>();
	
	public PlaySkyRushMenu(GuiUtils utils) {
		this.utils = utils;
	}
	...
	public void contents(Player player, Inventory inv) {
		...
		onelore.add("§8- §7Joueurs en jeu : §a" + CoreMain.getInstance().getServerManager().getPlayingPlayers(ServerGroup.skyrush2x1));
		onelore.add("");
		onelore.add("§6» §eClic-Gauche : §7Rejoindre le jeu");
		onelore.add("§6» §eClic-Droit : §7Afficher les serveurs");
		
		ItemStack itemStack = utils.createItem(Material.MAP, "§6SkyRush §8- §e§l1vs1", (byte) 0, onelore);
		itemStack.setAmount(1);
		inv.setItem(21, itemStack);
		serversGroups.put(21, ServerGroup.skyrush2x1);
		
		
		List<String> twolore = new ArrayList<>();
		twolore.add("");
		twolore.add("§8- §7Serveurs disponibles : §e" + CoreMain.getInstance().getServerManager().getServersOfGroup(ServerGroup.skyrush2x2).size());
		twolore.add("§8- §7Joueurs en jeu : §a" + CoreMain.getInstance().getServerManager().getPlayingPlayers(ServerGroup.skyrush2x2));
		twolore.add("");
		twolore.add("§6» §eClic-Gauche : §7Rejoindre le jeu");
		twolore.add("§6» §eClic-Droit : §7Afficher les serveurs");
		
		ItemStack itemStack2 = utils.createItem(Material.MAP, "§6SkyRush §8- §e§l2vs2", (byte) 0, twolore);
		itemStack2.setAmount(2);
		inv.setItem(22, itemStack2);
		serversGroups.put(22, ServerGroup.skyrush2x2);
		
		List<String> fourlore = new ArrayList<>();
		fourlore.add("");
		fourlore.add("§8- §7Serveurs disponibles : §e" + CoreMain.getInstance().getServerManager().getServersOfGroup(ServerGroup.skyrush2x4).size());
		fourlore.add("§8- §7Joueurs en jeu : §a" + CoreMain.getInstance().getServerManager().getPlayingPlayers(ServerGroup.skyrush2x4));
		fourlore.add("");
		fourlore.add("§6» §eClic-Gauche : §7Rejoindre le jeu");
		fourlore.add("§6» §eClic-Droit : §7Afficher les serveurs");
		
		ItemStack it3 = utils.createItem(Material.MAP, "§6SkyRush §8- §e§l4vs4", (byte) 0, fourlore);
		it3.setAmount(4);
		inv.setItem(23, it3);
		serversGroups.put(23, ServerGroup.skyrush2x4);
		...
		inv.setItem(4, MinigamesIconsHandler.getSkyRushIcon(false));
		//test
	}
